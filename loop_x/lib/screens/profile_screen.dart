...
class _ProfileScreenState extends State<ProfileScreen>{
  ...
  try {
    ...
    final postsResponse = await supabase
        .from('posts')
        .select('text, image_url, profiles(username, avatar_url)')
        .eq('user_id', userId)
    ...
    setState(() {
      ...
      bio = response['bio'] ?? '';
      profileImageUrl = response['avatar_url'] ?? ''; // Changed from profile_image_url
      followersCount = follower.length;
      followingCount = following.length;
      postResponse = List<Map<String, dynamic>>.from(postsResponse);
    });
  } catch (error) {
                ...
                Expanded(
                  child: TabBarView(
                    children: [
                      ...
                      Center(child: Text(' tweets will be displayed here')),
                    ],
                  ),
                ),
              ],
